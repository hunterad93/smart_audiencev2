            ...
            with st.spinner("Analyzing your input..."):
                if not group["assistant_id"]:
                    ...
                    if segments:
                        ...
                        StateService.set_group_assistant(group_id, openai_service.acuity_demo_assistant_id)
                        display_group_definition(f"```json\n{json.dumps(segments, indent=2)}\n```", group)
                        st.rerun()
                    else:
                        # Non-demographic flow - set assistant and send first message
                        assistant_id = openai_service.get_assistant_for_classification(
                            classification,
                            kpi_metric=st.session_state.selected_kpi
                        )
                        print(st.session_state.selected_kpi)
...
def classify_and_select_assistant(prompt: str, openai_service: OpenAIService) -> str:
    try:
        ...
        if segments:
            ...
            group.update(segments)
            return openai_service.acuity_demo_assistant_id
